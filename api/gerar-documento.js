...
const DOC_INSTRUCOES = {
  ...
  prompt_site_claude_code: 'Escreva um briefing completo para ser entregue ao Claude Code implementar este site do zero. Comece SEMPRE com um título "# Novo projeto Desiberne Studio — {nome do cliente}" seguido de uma seção "## Requisitos técnicos" com estes 4 itens exatos: repositório novo e privado na organização desiberneia-netizen no GitHub; site estático sem painel de edição pro cliente; deploy na Vercel gerando o quanto antes o link de prévia (.vercel.app) pra mostrar ao cliente antes de qualquer domínio; seguir a direção visual descrita evitando visual genérico de IA (gradiente roxo-azul padrão, Inter como escolha automática, cards centralizados com ícone sem motivo). Depois disso, estruture o resto em seções: Negócio, Direção visual, Mídia disponível, Estrutura de páginas com o que cada uma precisa conter, Requisitos especiais, Critérios de aceite. Seja concreto, não genérico. Markdown.',
}
